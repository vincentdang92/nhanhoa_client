    ...
    <a-row :gutter="24">
      <a-col v-for="(value, key) in data" :key="key" :span="12">
        <a-form-item :label="value.title">
          <a-input v-if="value.type == 'text'" :type="'text'" :disabled=value.disabled v-decorator="[value.decorator[0],{...value.decorator[1], initialValue: initialValues[key] }]" :placeholder="`Nhập ${value.title}`" />
          <a-select v-else label-in-value :disabled=value.disabled v-decorator="[value.decorator[0],{...value.decorator[1], initialValue: { key: initialValues[key]} }]" :placeholder="`Nhập ${value.title}`">
...
const data = {
  ...
  prefix: {
    ...
    disabled: true,
    type: 'text',
    decorator: accountsValidation.prefixPhone
  },
  phone: {
    title: "Số điện thoại",
    disabled: false,
  ...
  role: {
    ...
    decorator: accountsValidation.role
  },
...
export default {
  props: {
    initialValues: {
      ...
      default: () => {},
      validator: (value) =>
        ["id", "name", "email", "phone", "role", "prefix"].every((key) => key in value)
    }
  },
        ...
        if (!error) {
          ...
          const doAdd = await this.$store.dispatch(
            "accounts/update",
            {
              ...
              prefix_phone: this.initialValues.prefix
            }
          );
