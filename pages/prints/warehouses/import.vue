<template>
  <div>
    <div class="head-printer">
      <table>
        <tr>
          ...
          <td>
            <strong class="title"><img src="@/assets/imgs/checkbox.jpg"/>ĐƠN TRẢ LIỆU VỀ KHO</strong><br/>
            <strong class="title"><img src="@/assets/imgs/checkbox.jpg"/>ĐƠN NHẬP KHO</strong>
          </td>
        </tr>
      </table>
    </div>
      ...
      <thead>
        <tr v-if="warehouse_import_code">
          <th width="490" class="date">
            ...
            Mã kho: {{warehouseImport.warehouses.code}}<span></span>
          </th>
          <th class="date">Ngày tháng: <span></span>{{dateImport[0]}}<span>/</span>{{dateImport[1]}}<span>/</span>{{dateImport[2]}}</th>
        </tr>
      </thead>
    </table>
    <table class="tb-detail" border="1">
      ...
      <tbody>
        <tr v-for="(value, key) in warehouseImport.warehouse_import_detail" :key="key">
          <td>{{key + 1}}</td>
...
export default {
    ...
    async created() { 
      const data = await Promise.all([
        this.$store.dispatch('warehouseImport/show', {
          params: {
            id: this.$route.query.id
          }
        }),
      ]);
      this.warehouseImport = data[0];
      this.warehouse_import_code = this.warehouseImport.warehouse_import_code;
      let dateCustom = new Date(this.warehouseImport.created_at_publish);
      this.dateImport = [dateCustom.getDate(), dateCustom.getMonth()+1, dateCustom.getFullYear()];
      this.printer();
    },
    async fetch({ store }) {
      await Promise.all([
