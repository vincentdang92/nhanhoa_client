<template>
  <div>
    <div class="head-printer">
      <table>
        <tr>
          ...
          <td>
            <strong class="title"><img src="@/assets/imgs/checkbox.jpg"/>ĐIỀU KHO</strong>
          </td>
        </tr>
      </table>
    </div>
    ...
    <table class="tb-detail">
      <thead>
        <tr v-if="warehouse_import_code">
          <th width="490" class="date">
            Mã đơn: {{warehouse_import_code}}<span></span>
            Mã xuất: {{warehouse_from.code}}<span></span>
            Mã nhập: {{warehouseRotation.warehouses.code}}<span></span>
          </th>
          <th class="date">Ngày tháng: <span></span>{{dateRotation[0]}}<span>/</span>{{dateRotation[1]}}<span>/</span>{{dateRotation[2]}}</th>
        </tr>
      </thead>
    </table>
    <table class="tb-detail" border="1">
      ...
      <tbody>
        <tr v-for="(value, key) in warehouseRotation.warehouse_import_detail" :key="key">
          <td>{{key + 1}}</td>
        ...
        <tr v-if="warehouse_import_code">
          <td colspan="7" rowspan="3">
            Ghi Chú: {{warehouseRotation.title}}
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
    ...
    async created() { 
      const data = await Promise.all([
        this.$store.dispatch('warehouseRotation/show', {
          params: {
            id: this.$route.query.id
          }
        }),
      ]);
      this.warehouseRotation = data[0].details;
      this.warehouse_import_code = this.warehouseRotation.warehouse_import_code;
      this.warehouse_from = data[0].warehouse_from;
      let dateCustom = new Date(this.warehouseRotation.created_at_publish);
      this.dateRotation = [dateCustom.getDate(), dateCustom.getMonth()+1, dateCustom.getFullYear()];
      this.printer();
    },
    async fetch({ store }) {
      await Promise.all([
